...
use crate::tui::helpers::{pad_to_display_width, truncate_by_display_width};
...
pub(crate) fn draw_power_panel_v2(
    f: &mut Frame,
    area: Rect,
    s: &MetricsSnapshot,
    state: &AppState,
    ...
    let render_labeled_sparkline = |f: &mut Frame,
                                    area: Rect,
                                    label: &str,
                                    watts: f32,
                                    data: &[f64],
        ...
        for (row_idx, row) in graph.iter().enumerate() {
            ...
            if !spans.is_empty() {
                f.render_widget(
                    Paragraph::new(Line::from(spans)),
                    Rect::new(graph_area.x, y, graph_area.width, 1),
                );
            }
        }
    };
