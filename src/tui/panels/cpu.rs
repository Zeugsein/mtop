...
use crate::tui::{AppState, braille, gradient, layout, theme};
...
pub(crate) fn render_graph(
    ...
    let graph =
        braille::render_braille_graph(data, max, area.width as usize, area.height as usize, theme);
...
pub(crate) fn render_graph_green(
    f: &mut Frame,
    area: Rect,
    data: &[f64],
    max: f64,
    ...
    for (row_idx, row) in graph.iter().enumerate() {
        ...
        let t = if height > 1 {
            row_idx as f64 / (height - 1) as f64
        } else {
            1.0
        };
...
pub(crate) fn render_graph_with_baseline(
    f: &mut Frame,
    area: Rect,
    data: &[f64],
    max: f64,
...
pub(crate) fn draw_cpu_panel_v2(
    f: &mut Frame,
    area: Rect,
    s: &MetricsSnapshot,
    state: &AppState,
    ...
    let title_spans = vec![
        ...
        Span::styled(
            format!(
                " @ {}MHz",
                s.cpu.p_cluster.freq_mhz.max(s.cpu.e_cluster.freq_mhz)
            ),
            Style::default().fg(theme.muted),
        ),
        Span::styled(
    ...
    if state.show_detail {
        ...
        for (i, proc) in s.processes.iter().take(max_procs).enumerate() {
            ...
            let line = Line::from(vec![
                ...
                Span::styled(
                    "•",
                    Style::default().fg(gradient::value_to_color(cpu_norm, theme)),
                ),
                Span::raw(" "),
                Span::styled(
                    "•",
                    Style::default().fg(gradient::value_to_color(mem_norm, theme)),
                ),
                Span::raw(" "),
                Span::styled(
                    "•",
                    Style::default().fg(gradient::value_to_color(pow_norm, theme)),
                ),
            ]);
            f.render_widget(
                Paragraph::new(line),
