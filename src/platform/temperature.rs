...
/// Used on Apple Silicon (M4 Pro and later) where SMC may not expose temperature keys.
/// Pools all HID services that yield kIOHIDEventTypeTemperature events.
///
/// Known limitation: CPU and GPU cannot be distinguished without Product-name
/// classification (which requires Apple Vendor filter + entitlements not yet confirmed
/// for sandboxed mtop). Both averages report the same pooled value until resolved.
/// See hid_diag binary for hardware-level diagnostic output.
